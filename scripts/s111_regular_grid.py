...
def createGroup(hdf_file, netcdf_file, ugrid, vgrid, xgrid, ygrid, ncindex_xi1):
    ...
    if hdf_file.items() == []:
        ...
        for index in range(0, numberOfTimes):
            ...
            direction_dataset = newGroup.create_dataset('surfaceCurrentDirection', (vgrid.shape[0],vgrid.shape[1]), dtype=numpy.float32, data=directions,  chunks=True, compression="gzip", compression_opts=9,  fillvalue=-9999.0)
            speed_dataset = newGroup.create_dataset('surfaceCurrentSpeed', (ugrid.shape[0],ugrid.shape[1]), dtype=numpy.float32, data=speeds,  chunks=True, compression="gzip", compression_opts=9, fillvalue=-9999.0)
            groupXY = hdf_file.create_group("Group_XY")
            x_dataset = groupXY.create_dataset('Longitude', (ugrid.shape[0],ugrid.shape[1]), dtype=numpy.float32, data=xgrid, chunks=True, compression="gzip", compression_opts=9, fillvalue=-9999.0)
            y_dataset = groupXY.create_dataset('Latitude', (ugrid.shape[0],ugrid.shape[1]), dtype=numpy.float32, data=ygrid, chunks=True, compression="gzip", compression_opts=9, fillvalue=-9999.0)
